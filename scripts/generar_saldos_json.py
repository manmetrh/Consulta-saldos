...
records = df[['codigo', 'nombre', 'centro', 'almacen', 'libre', 'inspeccion', 'bloqueado', 'traslado', 'total', 'search']].to_dict(orient='records')
JSON_OUT.write_text(json.dumps(records, ensure_ascii=False, separators=(',', ':')), encoding='utf-8')
df[['codigo', 'nombre', 'centro', 'almacen', 'libre', 'inspeccion', 'bloqueado', 'traslado', 'total']].to_csv(CSV_OUT, index=False, encoding='utf-8')

print({'sheet': best_sheet, 'rows': int(len(df)), 'json': str(JSON_OUT), 'csv': str(CSV_OUT)})
